...
const courier = CourierClient(
	{ authorizationToken: "pk_prod_53105SYEB646Y7MQFBSGK7B8SQCV"});
...
const url= 'mongodb://localhost:27017/doc'
...
const users = [];
let findusers= async()=>{
	//const { recipient } = await courier.lists.subscribe("navya", `${uuidv4()}`);
...
io.on("connection", (socket) => {
  	...
  	socket.on("save-app",(doc,patient,date,time,desc)=>{
  		...
  		console.log(time);
  		  connect.then(async (db)  =>  {
  		  	let meet= `https://still-wave-47357.herokuapp.com/${uuidv4()}`
  	...
  	socket.on("prescribe",(doc,patient,symptoms,tests,advice,medicines,names)=>{
  		 connect.then(async (db)  =>  {
  		 	/*console.log(patient);
			   ...
			   let p= new Prescription({from:doc,for:patient,symptoms,tests,advice,medicines,names});
			   await p.save();
			   console.log("here",p);
			});
  	});
...
app.get('/prescription',isLoggedIn,async function(req,res){
	let notifs = await Notif.find({for: req.user.username});
	notifs.reverse();
	res.render('prescription',{notifs, doc: req.user.username});
})
